...
**Domain:** `Creator / Podcast Ops` · **Suite:** `Media Creator Suite` · **Accent:** `#fb7185`
...
**Podcast Show Notes** is a standalone, browser-based AI workflow app for **Creator / Podcast Ops**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `audio`<br>Audio | text | Yes | — | Affects input: Audio. |
| `transcript`<br>transcript | text | Yes | — | Affects input: transcript. |
...
| `show_style`<br>show style | select | No | — | Affects customization: show style. |
| `host_guest`<br>host/guest | text | No | — | Affects customization: host/guest. |
| `platform`<br>platform | select | No | Meta Ads | Affects customization: platform. |
| `sponsor_rules`<br>sponsor rules | textarea | No | — | Affects customization: sponsor rules. |
| `timestamp_format`<br>timestamp format | select | No | — | Affects customization: timestamp format. |
| `tone`<br>tone | text | No | — | Affects customization: tone. |
| `quote_style`<br>quote style | select | No | — | Affects customization: quote style. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9147
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9147`. |
...
```bash
curl http://127.0.0.1:9147/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9147/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9147 uvicorn app.main:app --host 0.0.0.0 --port 9147
```
...
- `plugins/podcast-show-notes.json` for plugin metadata.
